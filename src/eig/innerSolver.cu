...
void innerSolver_init(double *X, int ldX, double *R, int ldR, 
   ...
   /* 
      need to create diagonal matrix for both precisions
      need to alloc gpu memory for internal usage
   */
...
void innerSolver_destroy(struct jdqmr16Info *jd){
   ...
   /*
      free gpu memory for internal usage
   */
...
void innerSolver(double *X, int ldX, double *R, int ldR, 
   ...
   cudaMemcpy(X,R,dim*numEvals*sizeof(double),cudaMemcpyDeviceToDevice);
